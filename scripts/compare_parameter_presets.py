...
# Constraint presets are now imported from src.optimization.solvers.param_presets
# Use CONSTRAINT_PRESETS from that module (already imported above)
PRESETS: dict[str, OptimizationSettings] = CONSTRAINT_PRESETS

# Solver-specific preset options are now imported from src.optimization.solvers.param_presets
# Use SOLVER_PRESETS from that module (already imported above)


def _settings_from_dict(d: dict[str, Any]) -> OptimizationSettings:
    """Build OptimizationSettings from a dict (e.g. from JSON).
    
    Uses the centralized settings_from_dict function from param_presets.
    """
    return settings_from_dict(d)
...
def _product_label(data: Any, product_id: str) -> str:
    """Display name for product (English, for terminal) or ID if not available."""
    return (getattr(data, "product_names_en", None) or {}).get(product_id, product_id)


def _product_label_persian(data: Any, product_id: str) -> str:
    """Display name for product (Persian, for CSV) or ID if not available."""
...
def select_solver_preset_interactive(solver_name: str) -> dict[str, Any] | None:
    ...
    prompt = f"Select preset (1-{len(available_presets)}, or Enter to skip): "
    ...
    if selected_preset:
        preset_opts = get_preset_options(solver_name, selected_preset)
        if preset_opts:
...
def main() -> None:
    ...
    print("-" * 188)
    print(f"{'Preset':<20} {'Status':<12} {'Optimal':<8} {'Objective':<12} {'Time(s)':<10} {'Total delivery':<15} {'Slack: Demand':<18} {'Slack: Target':<15} {'Slack: Del Low':<18} {'Slack: Del High':<18}")
    print("-" * 188)
    for r in results:
        ...
        "presets": [
            {
                ...
                "settings": {
                    ...
                    "weight_demand": int(round(r["settings"].weight_demand)),
