/*
Purpose: Seed [Data].[dq_RuleCatalog] with initial Factory Inventory data quality rules.
Assumptions: T-SQL on SQL Server; [Data] schema and [Data].[dq_RuleCatalog] table exist (see 040_dq_tables.sql); requires permissions to INSERT/UPDATE into [Data].[dq_RuleCatalog].
...
USING (
    ...
    SELECT N'FI_EMPTY_LOAD', N'Data.stg_FactoryInventory', N'HARD', 
           ...
           1
    UNION ALL
    SELECT N'FI_NULL_QTY', N'Data.stg_FactoryInventory', N'HARD', 
...
SELECT 
    ...
    created_at
FROM [Data].[dq_RuleCatalog]
WHERE target_object = N'Data.stg_FactoryInventory'
ORDER BY rule_name;
