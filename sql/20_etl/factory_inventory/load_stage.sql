...
Usage: This stored procedure is typically called as part of an ETL pipeline after starting a batch (via ctl.usp_start_batch). It loads all current factory inventory rows from the source system into staging, tagged with the provided batch_id. This allows multiple batch runs to coexist in staging without affecting each other.
Parameters:
...
How to run: Execute via EXEC etl.usp_load_stage_factory_inventory @batch_id = 123. Should be called after starting a batch and before running validation/transformation steps.
*/
...
EXEC etl.usp_load_stage_factory_inventory @batch_id = 123;
