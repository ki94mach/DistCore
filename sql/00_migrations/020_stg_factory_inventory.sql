...
-- Create [$(prod_schema)].[stg_FactoryInventory] if missing
IF OBJECT_ID(N'[$(prod_schema)].[stg_FactoryInventory]', 'U') IS NULL
BEGIN
    CREATE TABLE [$(prod_schema)].[stg_FactoryInventory] (
...
IF EXISTS (
    SELECT 1
    FROM sys.columns c
    WHERE c.object_id = OBJECT_ID(N'[$(prod_schema)].[stg_FactoryInventory]', 'U')
      AND c.name = N'factory_id' AND c.is_nullable = 0
)
BEGIN
    ALTER TABLE [$(prod_schema)].[stg_FactoryInventory]
...
IF NOT EXISTS (
    SELECT 1
    FROM sys.indexes i
    WHERE i.object_id = OBJECT_ID(N'[$(prod_schema)].[stg_FactoryInventory]', 'U')
      AND i.name = N'CI_FactoryInventory_Batch_Factory_Product'
)
BEGIN
    CREATE CLUSTERED INDEX CI_FactoryInventory_Batch_Factory_Product
        ON [$(prod_schema)].[stg_FactoryInventory] (batch_id, factory_id, product_id);
END;
GO
...
BEGIN
    CREATE NONCLUSTERED INDEX IX_FactoryInventory_Factory_Product_Incl
        ON [$(prod_schema)].[stg_FactoryInventory] (factory_id, product_id)
