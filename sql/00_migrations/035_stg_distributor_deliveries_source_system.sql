...
Assumptions: [$(prod_schema)].[stg_DistributorDeliveries] already exists (024_stg_distributor_deliveries.sql).
How to run: Execute in SSMS or via sqlcmd against the target database; script is idempotent.
*/

IF OBJECT_ID(N'[$(prod_schema)].[stg_DistributorDeliveries]', 'U') IS NOT NULL
BEGIN
    IF EXISTS (
        SELECT 1
        ...
        WHERE dc.parent_object_id = OBJECT_ID(N'[$(prod_schema)].[stg_DistributorDeliveries]', 'U')
          AND c.name = N'source_system'
          AND dc.name = N'DF_DistributorDeliveries_source_system'
    )
    BEGIN
        ALTER TABLE [$(prod_schema)].[stg_DistributorDeliveries]
