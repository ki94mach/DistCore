...
Aggregation Logic: 
    ...
    - Result: Total inventory quantity per product across all distributors for today's date
Grain: One row per (snapshot_date, product_id) in the snapshot table. Quantities are summed across all distributors.
...
BEGIN
    ...
    -- IMPORTANT: This aggregation sums inventory quantities across ALL distributors for each product
    -- Grain: One row per (product_id, distributor_id) combination
    -- Aggregation rule: SUM(on_hand_qty) - sums on_hand_qty across all distributors for each (product_id, distributor_id) combination
