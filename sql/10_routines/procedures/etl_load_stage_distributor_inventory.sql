/*
Purpose: Load distributor inventory data from source table [DWOrchid].[dbo].[FactInventory] into staging table [Data].[stg_DistributorInventory] for a given batch.
Assumptions: T-SQL on SQL Server; source table [DWOrchid].[dbo].[FactInventory] exists and is accessible; staging table [Data].[stg_DistributorInventory] already exists (see 020_stg_distributor_inventory.sql); @batch_id corresponds to a valid batch run in [Data].[ctl_BatchRun].
Usage: This stored procedure is typically called as part of an ETL pipeline after starting a batch (via [Data].[ctl_usp_start_batch]). It loads distributor inventory rows from the source system into staging, tagged with the provided batch_id. Supports incremental loading by default (only loads records after the latest date in staging). This allows multiple batch runs to coexist in staging without affecting each other.
Parameters:
...
How to run: Execute via EXEC [Data].[etl_usp_load_stage_distributor_inventory] @batch_id = 123. Should be called after starting a batch and before running snapshot transformation steps.
*/
...
-- Staging Table: [Data].[stg_DistributorInventory]
--
-- Mapping:
--   batch_id              -> @batch_id (parameter, applied to all rows)
--   [FkDistributor]        -> distributor_id (INT)
--   [FkCenter]        -> center_id (INT)
...
CREATE OR ALTER PROCEDURE [Data].[etl_usp_load_stage_distributor_inventory]
    @batch_id BIGINT,
    @incremental BIT = 1
AS
BEGIN
    ...
    DELETE FROM [Data].[stg_DistributorInventory] WHERE batch_id = @batch_id;
    ...
    -- The staging table [Data].[stg_DistributorInventory] must exist in the source database.
    ...
    INSERT INTO [Data].[stg_DistributorInventory] (batch_id, distributor_id, center_id, product_id, product_batch_no, as_of_datetime, on_hand_qty)
    SELECT
        @batch_id,
        CAST([FkDistributor] AS INT)      AS distributor_id,         
        CAST([FkCenter] AS INT)      AS center_id,         
...
EXEC [Data].[etl_usp_load_stage_distributor_inventory] @batch_id = 123;

-- Example 1b: Load staging data for batch 123 (full load, all records)
EXEC [Data].[etl_usp_load_stage_distributor_inventory] @batch_id = 123, @incremental = 0;
...
SELECT 
    batch_id,
    COUNT(*) AS total_rows,
    SUM(CASE WHEN distributor_id IS NULL OR center_id IS NULL OR product_id IS NULL THEN 1 ELSE 0 END) AS null_key_rows
FROM [Data].[stg_DistributorInventory]
WHERE batch_id = 123
GROUP BY batch_id;
*/
